...
import json  # need to change the format to store more information then just names
...
class Block(object):  # declare individual blocks
    ...
    def __repr__(self):
        return "{} - {} - {} - {} - {}".format(self.index, self.proof_number, self.previous_hash, self.data,
                                               self.timestamp)


class BlockChain(object):  # declare the
    def __init__(self):
        self.chain = []
        self.current_data = []
        self.nodes = set()
    ...
    def confirm_validity(block, previous_block):
        if previous_block.index + 1 != block.index:
            ...
            return False
        # flip the comparison (<=) and see if it works because time stamp of the
